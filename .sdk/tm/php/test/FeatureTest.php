...
class FtCtrl extends LetscountControl
{
...
class FtClient
{
    ...
    private LetscountUtility $utility;

    public function __construct(LetscountUtility $utility, array $options)
    {
    ...
    public function get_utility(): LetscountUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LetscountContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LetscountContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LetscountContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LetscountConfig::make_feature($name);
    ...
    private static function buildUrl(LetscountSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LetscountError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LetscountResponse($fetched);
        }
    ...
    private function fail(LetscountContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LetscountContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
