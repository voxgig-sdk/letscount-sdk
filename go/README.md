...
func main() {
    ...
    created, err := client.CreateOrUpdateCounter(nil).Create(map[string]any{"name": "Example"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(created)
...
createorupdatecounter, err := client.CreateOrUpdateCounter(nil).Load(
    map[string]any{"id": "test01"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(createorupdatecounter) // the loaded mock data
```
...
slice):
    ...
    createorupdatecounter, err := client.CreateOrUpdateCounter(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
    // createorupdatecounter is the loaded record
...
```go
get_counter, err := client.GetCounter(nil).Load(map[string]any{"id": "get_counter_id"}, nil)
if err != nil {
    panic(err)
}
...
createorupdatecounter.Load(map[string]any{"id": "example_id"}, nil)
