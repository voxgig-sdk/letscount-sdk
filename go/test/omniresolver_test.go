...
import (
	...
	omni "github.com/voxgig-sdk/letscount-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LetscountError); is {
				out := map[string]any{
					"name":    "LetscountError",
...
func omniCtx(arg any, client *sdk.LetscountSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LetscountSDK); is {
			client = live
