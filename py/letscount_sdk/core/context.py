...
from letscount_sdk.core.helpers import get_ctx_prop, to_map
...
class LetscountContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, LetscountSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, LetscountResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, LetscountResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return LetscountError(code, msg, self)
