...
class DecrementCounterEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DecrementCounterEntity(self._client, opts)
    ...
    def data_get(self) -> DecrementCounter:
    ...
    def match_get(self) -> DecrementCounter:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DecrementCounter:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DecrementCounter().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
