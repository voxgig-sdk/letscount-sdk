...
module LetscountUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LetscountResponse.new({})
    ...
    if fetch_err
      ...
      response = LetscountResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LetscountResponse.new(fetched)
    else
