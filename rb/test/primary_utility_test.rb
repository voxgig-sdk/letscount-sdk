...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = LetscountOmni.make_runner(TEST_JSON_FILE, LetscountSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : LetscountOmni.errify(err)
    end
