...
class LetscountSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LetscountHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LetscountHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LetscountFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LetscountUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = LetscountHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LetscountHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = LetscountHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LetscountError.new(
        "graphql_error", "LetscountSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
