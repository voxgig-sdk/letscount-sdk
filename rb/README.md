...
The Ruby SDK for the Letscount API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.CreateOrUpdateCounter(nil).create({ "name" => "Example" }, nil)
...
result, err = client.Letscount(nil).load(
  { "id" => "test01" }, nil
)
