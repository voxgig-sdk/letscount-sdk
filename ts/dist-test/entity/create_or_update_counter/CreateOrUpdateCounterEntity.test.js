...
(0, node_test_1.describe)('CreateOrUpdateCounterEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'create_or_update_counter.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['create_or_update_counter01', 'create_or_update_counter02', 'create_or_update_counter03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
