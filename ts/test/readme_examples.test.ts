...
//     (\`new LetscountSDK(...)\` / \`LetscountSDK.test(...)\` become
//     \`LetscountSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"create_or_update_counter":{"test01":{"id":"test01"}},"decrement_counter":{"test01":{"id":"test01"}},"get_counter":{"test01":{"id":"test01"}},"increment_counter":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new LetscountSDK(...)\` and any
// \`LetscountSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+LetscountSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/LetscountSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
