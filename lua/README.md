...
The Lua SDK for the Letscount API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:CreateOrUpdateCounter(nil):create({ name = "Example" }, nil)
...
local result, err = client:Letscount(nil):load(
  { id = "test01" }, nil
)
