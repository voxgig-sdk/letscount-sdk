...
describe("DecrementCounterEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:DecrementCounter(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = decrement_counter_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "decrement_counter." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LETSCOUNT_TEST_DECREMENT_COUNTER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local decrement_counter_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.decrement_counter")))
    local decrement_counter_ref01_data = nil
    if #decrement_counter_ref01_data_raw > 0 then
...
function decrement_counter_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/decrement_counter/DecrementCounterTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read decrement_counter test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LETSCOUNT_TEST_DECREMENT_COUNTER_ENTID")
