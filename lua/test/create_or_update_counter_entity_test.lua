...
describe("CreateOrUpdateCounterEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:CreateOrUpdateCounter(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = create_or_update_counter_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "create_or_update_counter." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LETSCOUNT_TEST_CREATE_OR_UPDATE_COUNTER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local create_or_update_counter_ref01_ent = client:CreateOrUpdateCounter(nil)
    local create_or_update_counter_ref01_data = helpers.to_map(vs.getprop(
...
function create_or_update_counter_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/create_or_update_counter/CreateOrUpdateCounterTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read create_or_update_counter test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LETSCOUNT_TEST_CREATE_OR_UPDATE_COUNTER_ENTID")
