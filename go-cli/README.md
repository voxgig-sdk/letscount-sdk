...
boru-driven command-line client **and** interactive REPL for the Letscount
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/letscount-cli)
make build
...
LETSCOUNT_BASE=https://api.example.com ./letscount-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/letscount-cli
   ```
...
Run with no arguments to open a REPL (prompt `letscount>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `create_or_update_counter`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/letscount-cli`. |
...
(`./letscount-cli <expr>`) and interactively (the REPL), and expressions compose the
