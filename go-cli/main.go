// Command Letscount-cli is a CLI + REPL for the Letscount SDK,
...
const prompt = "letscount"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "create_or_update_counter decrement_counter get_counter increment_counter"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LETSCOUNT_APIKEY (api key), LETSCOUNT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LETSCOUNT_APIKEY carries the API key and
	// LETSCOUNT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LETSCOUNT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LETSCOUNT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLetscountSDK(opts)
