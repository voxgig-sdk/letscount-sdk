...
The LetsCount API allows users to create, retrieve, update, increment, and decrement counters identified by a unique namespace and key.

Learn more about LetsCount API at [letscountapi.com](https://letscountapi.com/).

This is an unofficial SDK for the LetsCount public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — CreateOrUpdateCounter, DecrementCounter, GetCounter and IncrementCounter — that you
...
const createorupdatecounter = await client.CreateOrUpdateCounter().create({
  key: 'example',
  namespace: 'example',
})
```
...
```go
client := sdk.Test()
result, err := client.GetCounter(nil).Load(
    nil, nil,
)
```
...
| --- | --- | --- |
| **CreateOrUpdateCounter** | The CreateOrUpdateCounter entity (create). | `/{namespace}/{key}` |
| **DecrementCounter** | The DecrementCounter entity (remove). | `/{namespace}/{key}` |
| **GetCounter** | The GetCounter entity (load). | `/{namespace}/{key}` |
| **IncrementCounter** | The IncrementCounter entity (update). | `/{namespace}/{key}` |
