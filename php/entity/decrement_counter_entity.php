...
// Letscount SDK DecrementCounter entity
...
class DecrementCounterEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DecrementCounterEntity($this->_client, $opts);
    }

    /**
     * @param DecrementCounter|array $args DecrementCounter data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LetscountHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DecrementCounter|array The current DecrementCounter data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DecrementCounter fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LetscountHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DecrementCounter fields).
     */
     ...
     *
     * @param DecrementCounterRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; DecrementCounterRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DecrementCounter|array The removed DecrementCounter as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LetscountHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
