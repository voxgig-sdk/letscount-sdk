...
{
    private static function utility(): LetscountUtility
    {
        return new LetscountUtility();
    }
    ...
    private static function ctx(array $over = []): LetscountContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new LetscountContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new LetscountOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof LetscountError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new LetscountOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = LetscountMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new LetscountOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = LetscountMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new LetscountOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = LetscountMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LetscountSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = LetscountMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new LetscountResponse([]),
            'result' => new LetscountResult([]),
        ]);
        [, $err] = LetscountMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new LetscountSpec([]),
            'response' => null,
            'result' => new LetscountResult([]),
        ]);
        [, $err] = LetscountMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new LetscountSpec(['step' => 's']),
            'response' => new LetscountResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new LetscountResult([]),
        ]);
        [, $err] = LetscountMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new LetscountSpec(['step' => 's']),
            'response' => new LetscountResponse(self::resp(200, ['v' => 1])),
            'result' => new LetscountResult([]),
        ]);
        [, $err] = LetscountMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new LetscountSpec(['step' => 's']),
            'response' => new LetscountResponse(self::resp(200, ['v' => 2])),
            'result' => new LetscountResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        LetscountMakeResponse::call($ctx);
    ...
    {
        $preset = new LetscountResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new LetscountSpec([]),
            'response' => new LetscountResponse([]),
            'result' => new LetscountResult([]),
        ...
        [$got, $err] = LetscountMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new LetscountResult([])]);
        [, $err] = LetscountMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new LetscountSpec([]), 'result' => null]);
        [, $err] = LetscountMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LetscountSpec(['step' => 's']),
            'result' => new LetscountResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new LetscountOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LetscountMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LetscountSpec(['step' => 's']),
            'result' => new LetscountResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new LetscountOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LetscountMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LetscountResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new LetscountSpec([]),
            'result' => new LetscountResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = LetscountMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LetscountMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new LetscountError('boom', 'boom');
        $utility->fetcher = function (LetscountContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LetscountSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = LetscountMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (LetscountContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = LetscountMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(LetscountResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (LetscountContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LetscountSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = LetscountMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new LetscountResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new LetscountSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = LetscountMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = LetscountMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = LetscountMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new LetscountResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, LetscountDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new LetscountResult(['ok' => false])]);
        $this->expectException(LetscountError::class);
        LetscountDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new LetscountResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', LetscountMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        LetscountMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new LetscountBaseFeature();
        $b = new LetscountBaseFeature();
        LetscountFeatureAdd::call($ctx, $a);
        LetscountFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): LetscountBaseFeature
    {
        $f = new LetscountBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        LetscountFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): LetscountContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new LetscountSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LetscountPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new LetscountResponse(['headers' => null]),
            'result' => new LetscountResult([]),
        ]);
        LetscountResultHeaders::call($ctx);
