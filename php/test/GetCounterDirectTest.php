...
class GetCounterDirectTest extends TestCase
{
    public function test_direct_load_get_counter(): void
    {
        $setup = get_counter_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-get_counter", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function get_counter_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new LetscountSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
