...
class LetscountNetsimFeature extends LetscountBaseFeature
{
    ...
    private function _simulate(LetscountContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(LetscountContext $ctx, array $applied): void
    {
        $client = $this->client;
