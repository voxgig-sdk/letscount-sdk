...
type Args struct {
	Entity string         `json:"entity" jsonschema:"create_or_update_counter | decrement_counter | get_counter | increment_counter"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.LetscountSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "letscount_list",
		Description: "List records from Letscount. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "letscount_load",
		Description: "Load a single record from Letscount. " +
...
func runOp(client *sdk.LetscountSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.LetscountSDK, name string) (sdk.LetscountEntity, error) {
	switch strings.ToLower(name) {
	case "create_or_update_counter":
		return client.CreateOrUpdateCounter(nil), nil
	case "decrement_counter":
		return client.DecrementCounter(nil), nil
	case "get_counter":
		return client.GetCounter(nil), nil
	case "increment_counter":
		return client.IncrementCounter(nil), nil
