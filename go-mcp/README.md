...
[MCP](https://modelcontextprotocol.io) server exposing the Letscount SDK as
two agent tools — `letscount_list` and `letscount_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/letscount-mcp)
make build

# 2. Provide credentials via the environment
export LETSCOUNT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user letscount \
  -- /absolute/path/to/letscount-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./letscount-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// letscount_list: first page of records
{ "entity": "create_or_update_counter" }
{ "entity": "create_or_update_counter", "query": { } }

// letscount_load: one record by id
{ "entity": "get_counter", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/letscount-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user letscount \
     -- "$PWD"/dist/*/letscount-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `letscount_list` and `letscount_load` tools now appear
   in new sessions. Ask the agent to *"list create_or_update_counter using letscount"*
   and it calls `letscount_list` with `{"entity":"create_or_update_counter"}`.
...
```sh
export LETSCOUNT_APIKEY=sk_live_xxx            # API key
export LETSCOUNT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `letscount_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `letscount_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LETSCOUNT_APIKEY` | API key sent with every request. |
| `LETSCOUNT_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

create_or_update_counter | decrement_counter | get_counter | increment_counter

### Smoke test via HTTP (raw JSON-RPC)

```sh
./letscount-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"letscount_load","arguments":{"entity":"get_counter","query":{"id":1}}}}'
```
